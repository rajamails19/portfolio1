...
import { BookOpen, CircleDot, Code2, Search, Quote } from "lucide-react";
...
export function SectionView({ section }: { section: Section }) {
  ...
  const showTopics = deck.brandName === "ChaatDeck" && section.slug === "qans";
  const visibleItems = useMemo(
    () =>
      showTopics ? section.items.filter((item) => item.category === activeTopic) : section.items,
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 pb-16 pt-8">
      <div className="relative overflow-hidden rounded-4xl shadow-glow ring-1 ring-rose/20">
        {founder && (
          <img
            src={founder.image}
            alt=""
        ...
        <div className="absolute -right-16 -top-16 h-64 w-64 rounded-full bg-rose/20 blur-3xl" />
        <div className="relative grid gap-6 p-8 sm:p-10 md:grid-cols-[1.5fr,1fr] md:items-center">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-rose/30 bg-noir/60 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.25em] text-rose backdrop-blur">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-rose/25 bg-noir/60 px-3 py-1 text-xs font-semibold text-foreground/80">
          ...
          {founder && (
            <div className="glass-strong hidden rounded-3xl p-4 md:block">
              ...
              <Quote className="mt-3 h-4 w-4 text-rose" />
      ...
      {showTopics && (
        <div className="mt-5 flex justify-center sm:justify-start">
          <div
            className="glass inline-flex max-w-full items-center gap-1 overflow-x-auto rounded-full border border-rose/20 p-1.5 shadow-[var(--shadow-soft)]"
            role="tablist"
      ...
      <div className="glass sticky top-4 z-10 mt-6 flex items-center gap-3 rounded-full px-4 py-2.5">
        ...
        <input
          ...
          placeholder={`Search in ${showTopics ? activeTopic : section.title}…`}
          className="flex-1 bg-transparent text-sm placeholder:text-muted-foreground/70 focus:outline-none"
        />
        {q && (
          <button
            onClick={() => setQ("")}
            className="rounded-full border border-rose/25 bg-noir/60 px-2 py-0.5 text-xs font-medium text-foreground/80 hover:bg-noir"
          >
            clear
          </button>
        )}
