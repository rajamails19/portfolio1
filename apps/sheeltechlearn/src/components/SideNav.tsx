...
export function SideNav() {
  ...
  const deck = useDeck();
  const activeSlug = deck.sections.find((s) => location.pathname.startsWith(`/${s.slug}`))?.slug ?? "qans";
    ...
    <aside className="flex w-72 shrink-0 flex-col gap-6 p-6">
      <Link to="/" className="group flex items-center gap-3">
        <div className="relative">
          <span className="absolute -inset-1 rounded-2xl bg-gradient-to-br from-rose to-coral opacity-60 blur-md" />
        ...
        <div className="leading-tight">
          ...
          <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">{deck.brandTag}</div>
        </div>
      </Link>
          ...
          return (
            <Link
              ...
              className={[
                ...
                active
                  ? "bg-gradient-to-r from-rose/90 to-coral/80 text-primary-foreground shadow-glow"
      ...
      <div className="glass animate-glow-pulse relative overflow-hidden rounded-3xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate font-display text-sm font-semibold text-rose">{activeFounder.name}</div>
