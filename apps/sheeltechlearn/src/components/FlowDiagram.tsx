...
export function FlowDiagram({ block }: { block: FlowBlock }) {
  ...
  return (
    <figure className="my-5 overflow-hidden rounded-2xl border border-rose/20 bg-noir/50 p-5">
      {block.title && (
        <figcaption className="mb-4 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.25em] text-rose/80">
          <span className="h-px flex-1 bg-gradient-to-r from-transparent to-rose/40" />
          {block.title}
          <span className="h-px flex-1 bg-gradient-to-l from-transparent to-rose/40" />
        </figcaption>
      )}
      <div
        className={[
          ...
          return (
            <div key={i} className={horizontal ? "flex items-center gap-3" : "flex flex-col items-center gap-2"}>
              ...
              {i < block.nodes.length - 1 && (
                <Arrow className="h-5 w-5 shrink-0 text-rose/70" strokeWidth={2.5} />
              )}
            </div>
          );
        })}
